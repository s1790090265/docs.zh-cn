...
> **说明**
>
> - 自 3.1 版本起，StarRocks 支持随机分桶，可以将数据随机地分布在所有分桶中。您在建表和新增分区时无需设置分桶键，提升建表语句的易用性。更多信息，请参见[随机分桶](#随机分桶自-31)。
...
StarRocks 支持如下数据分布方式：

- Hash 数据分布方式：一张表为一个分区，分区按照分桶键和分桶数量进一步进行数据划分。

- Range+Hash 数据分布方式：一张表拆分成多个分区，每个分区按照分桶键和分桶数量进一步进行数据划分。

- Random 数据分布：一张表的数据随机分布至不同分桶。

- Range+Random 数据分布：表的数据按照分区列值的范围划分成不同分区。同一分区的数据随机分布至不同分桶。

例如，采用 Hash 分布的建表语句如下，其中分桶键为 `site_id`：
...
分区的下一级是分桶，StarRocks 支持[哈希分桶](#哈希分桶)。在同一分区内，分桶键哈希值相同的数据形成 Tablet，Tablet 以多副本冗余的形式存储，是数据均衡和恢复的最⼩单位。Tablet 的副本由一个单独的本地存储引擎管理，数据导入和查询最终都下沉到所涉及的 Tablet 副本上。自 3.1 版本起，StarRocks 还支持[随机分桶](#随机分桶自-31)，建表和新增分区时无需设置分桶键。

自 2.5.7 版本起，StarRocks 支持自动设置分桶数量，建表和新增分区时无需设置分桶数量。更多信息，请参见[确定分桶数量](#确定分桶数量)。
...
哈希分桶的优点是可以实现数据的均衡分布，尽量确保不同的数据被分配到不同的桶中，从而提高查询性能和系统的整体吞吐量。同时，相似的数据在一定程度上有可能被哈希到相同的桶中，提供了一定的数据局部性，查询操作通常只需要在该分桶中进行扫描，而不需要跨越多个分桶或节点。从而减少数据传输开销，提高查询效率。
...
对每个分区的数据，StarRocks 将数据随机地分布在所有分桶中，而不受到特定列值的影响。并且建表时无需设置分桶键，简化建表语句。适用于数据量不大，对查询性能要求不高的场景。
...
如下示例中，创建表 `site_access1`，使用随机分桶并且由系统自动设置分桶数量。
