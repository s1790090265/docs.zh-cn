...
> **说明**
>
> - 自 3.1 版本起，您在建表和新增分区时可以不设置分桶键。StarRocks 会使用默认的随机分桶，将数据随机地分布在分区的所有分桶中。更多信息，请参见[随机分桶](#随机分桶自-31)。
> - 自 2.5.7 版本起，您无需手动设置分桶数量。StarRocks 支持在建表和新增分区时自动设置分桶数量 (BUCKETS)，如果自动设置分桶数量后性能未能达到预期，并且您比较熟悉分桶机制，则您也可以[手动设置分桶数量](#确定分桶数量)。
